  ...
  if(view==="admin"){
    ...
    return(<div style={{...ss.page,paddingBottom:16}}>{CSS}{Toast}
      ...
      {adminTab==="records"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        {(()=>{
          const dayRecs=(records[filterDate]||[]).sort((a,b)=>a.time-b.time);
          const empIds=[...new Set(dayRecs.map(r=>r.empId))];
          if(!empIds.length)return<div style={{textAlign:"center",padding:20,fontFamily:font,fontSize:12,color:C.dim}}>Sin registros</div>;
          return empIds.map(empId=>{
            const emp=employees.find(e=>e.id===empId);
            const col=getAvatarColor(empId);
            const empRecs=dayRecs.filter(r=>r.empId===empId);
            const worked=getWorked(records,empId,filterDate);
            return(<div key={empId} style={{...ss.card,padding:0,overflow:"hidden"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:C.cardLight,borderBottom:`1px solid ${C.border}`}}>
                <div style={ss.avatar(col,34)}>{emp?.name.split(" ").map(n=>n[0]).join("").slice(0,2)}</div>
                <div style={{flex:1}}><div style={{fontFamily:font,fontSize:13,fontWeight:700}}>{emp?.name.split(" ")[0]}</div><div style={{fontFamily:font,fontSize:10,color:C.muted}}>{emp?.position||"—"}</div></div>
                {worked>0&&<div style={{fontFamily:font,fontSize:14,fontWeight:700,color:C.accent}}>{fmtDur(worked)}</div>}
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:0}}>
                {empRecs.map((r,i)=><div key={r.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:i<empRecs.length-1?`1px solid ${C.border}22`:"none",background:editRecId===r.id?C.accent+"08":"transparent"}}>
        ...
        {!(records[filterDate]||[]).length&&null}
      </div>}
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",borderBottom:`1px solid ${C.border}`,background:C.cardLight}}>
                  ...
                  <button onClick={()=>{
                    ...
                    weekDays.forEach((d,i)=>{
                      ...
                      const shifts=Array.isArray(raw)?raw:raw?.start?[raw]:[];
                      if(shifts.length>0)data[i]=shifts;
                    });
                    setCopiedWeek({empId:emp.id,empName:emp.name.split(" ")[0],data});
                    flash(`Semana de ${emp.name.split(" ")[0]} copiada ✓`);
                  }} style={{fontFamily:font,fontSize:10,padding:"4px 10px",borderRadius:7,border:`1px solid ${C.border}`,background:C.cardLight,color:C.purple,cursor:"pointer",fontWeight:700}}>📋 Copiar</button>
                  {copiedWeek&&<button onClick={async()=>{
                    const ns={...schedules};let count=0;
                    for(const [dayOffset,shifts] of Object.entries(copiedWeek.data)){
                      const targetDay=weekDays[parseInt(dayOffset)];
                      if(!targetDay)continue;
                      await DB.deleteSchedule(emp.id,targetDay.date);
                      for(let si=0;si<shifts.length;si++){await DB.setSchedule(emp.id,targetDay.date,shifts[si].start,shifts[si].end,si);}
                      ns[emp.id+"_"+targetDay.date]=shifts;count++;
                    }
                    setSchedules(ns);
                    flash(`Pegado en ${emp.name.split(" ")[0]} ✓`);
                  }} style={{fontFamily:font,fontSize:10,padding:"4px 10px",borderRadius:7,border:"none",background:C.purple,color:"#fff",cursor:"pointer",fontWeight:700}}>📋 Pegar</button>}
                </div>
